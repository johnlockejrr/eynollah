...
def return_x_start_end_mothers_childs_and_type_of_reading_order(x_min_hor_some, x_max_hor_some, cy_hor_some, peak_points, cy_hor_diff):
        ...
        if type(all_args_uniq[0]) is list:
            for dd in range(len(all_args_uniq)):
                if len(all_args_uniq[dd]) == 2:
                    ...
                    # print('burda2')
                    if x_s_same_hor[0] == (x_e_same_hor[1] - 1) or x_s_same_hor[1] == (x_e_same_hor[0] - 1) and \
                            x_s_same_hor[0] != x_s_same_hor[1] and x_e_same_hor[0] != x_e_same_hor[1]:
                        # print('burda3')
    ...
    return reading_order_type, x_start_returned, x_end_returned, y_sep_returned, y_diff_returned, y_lines_without_mother, x_start_without_mother, x_end_without_mother, there_is_sep_with_child, y_lines_with_child_without_mother, x_start_with_child_without_mother, x_end_with_child_without_mother, new_main_sep_y
...
def find_features_of_lines(contours_main):
    ...
    return slope_lines, dis_x, x_min_main, x_max_main, np.array(cy_main), np.array(
        slope_lines_org), y_min_main, y_max_main, np.array(cx_main)
...
def boosting_headers_by_longshot_region_segmentation(textregion_pre_p, textregion_pre_np, img_only_text):
    ...
    textregion_pre_p[:, :, 0][
        (img_only_text[:, :] == 1) & (textregion_pre_p[:, :, 0] != 7) & (textregion_pre_p[:, :, 0] != 4) & (
...
def find_num_col(regions_without_separators, num_col_classifier, tables, multiplier=3.8):
    ...
    peaks = peaks[
        (peaks > 0.06 * regions_without_separators.shape[1]) & (peaks < 0.94 * regions_without_separators.shape[1])]
    ...
    if num_col == 3:
        if (peaks_neg_fin[0] > p_g_u and peaks_neg_fin[1] > p_g_u) or (
                peaks_neg_fin[0] < p_g_l and peaks_neg_fin[1] < p_g_l) or (
                (peaks_neg_fin[0] + 200) < p_m and peaks_neg_fin[1] < p_m) or (
                (peaks_neg_fin[0] - 200) > p_m and peaks_neg_fin[1] > p_m):
            num_col = 1
            peaks_neg_fin = []
    ...
    # print(num_col,'early')
    if num_col == 3:
        if (peaks_neg_true[0] > p_g_u and peaks_neg_true[1] > p_g_u) or (
                peaks_neg_true[0] < p_g_l and peaks_neg_true[1] < p_g_l) or (
                peaks_neg_true[0] < p_m and (peaks_neg_true[1] + 200) < p_m) or (
                (peaks_neg_true[0] - 200) > p_m and peaks_neg_true[1] > p_m):
            num_col = 1
            peaks_neg_true = []
...
def find_num_col_only_image(regions_without_separators, multiplier=3.8):
    ...
    peaks = peaks[
        (peaks > 0.09 * regions_without_separators.shape[1]) & (peaks < 0.91 * regions_without_separators.shape[1])]
    ...
    if num_col == 3:
        if (peaks_neg_fin[0] > p_g_u and peaks_neg_fin[1] > p_g_u) or (
                peaks_neg_fin[0] < p_g_l and peaks_neg_fin[1] < p_g_l) or (
                peaks_neg_fin[0] < p_m and peaks_neg_fin[1] < p_m) or (
                peaks_neg_fin[0] > p_m and peaks_neg_fin[1] > p_m):
            num_col = 1
        else:
            pass
    ...
    if num_col == 3:
        if (peaks_neg_true[0] > p_g_u and peaks_neg_true[1] > p_g_u) or (
                peaks_neg_true[0] < p_g_l and peaks_neg_true[1] < p_g_l) or (
                peaks_neg_true[0] < p_m and peaks_neg_true[1] < p_m) or (
                peaks_neg_true[0] > p_m and peaks_neg_true[1] > p_m):
            num_col = 1
            peaks_neg_true = []
        elif (peaks_neg_true[0] < p_g_u and peaks_neg_true[0] > p_g_l) and (peaks_neg_true[1] > p_u_quarter):
            peaks_neg_true = [peaks_neg_true[0]]
    ...
    if num_col == 4:
        if len(np.array(peaks_neg_true)[np.array(peaks_neg_true) < p_g_l]) == 2 or len(
                np.array(peaks_neg_true)[np.array(peaks_neg_true) > (len(y) - p_g_l)]) == 2:
            num_col = 1
            peaks_neg_true = []
        else:
            pass
    ...
    for i in range(len(peaks_neg_true)):
        ...
        hills_around = peaks_neg_org[((peaks_neg_org > hill_main) & (peaks_neg_org <= hill_main + 400)) | (
                    (peaks_neg_org < hill_main) & (peaks_neg_org >= hill_main - 400))]
...
def check_any_text_region_in_model_one_is_main_or_header(regions_model_1, regions_model_full, contours_only_text_parent,
                                                         all_box_coord, all_found_textline_polygons, slopes,
...
def check_any_text_region_in_model_one_is_main_or_header_light(regions_model_1, regions_model_full,
                                                               contours_only_text_parent, all_box_coord,
    ...
    regions_model_1 = cv2.resize(regions_model_1,
                                 (int(regions_model_1.shape[1] / 3.), int(regions_model_1.shape[0] / 3.)),
                                 interpolation=cv2.INTER_NEAREST)
    regions_model_full = cv2.resize(regions_model_full,
                                    (int(regions_model_full.shape[1] / 3.), int(regions_model_full.shape[0] / 3.)),
                                    interpolation=cv2.INTER_NEAREST)
...
def combine_hor_lines_and_delete_cross_points_and_get_lines_features_back_new(img_p_in_ver, img_in_hor,
    ...
    slope_lines_ver, dist_x_ver, x_min_main_ver, x_max_main_ver, cy_main_ver, slope_lines_org_ver, y_min_main_ver, y_max_main_ver, cx_main_ver = find_features_of_lines(
        contours_lines_ver)
    ...
    slope_lines_hor, dist_x_hor, x_min_main_hor, x_max_main_hor, cy_main_hor, slope_lines_org_hor, y_min_main_hor, y_max_main_hor, cx_main_hor = find_features_of_lines(
        contours_lines_hor)
    ...
    len_lines_bigger_than_x_width_smaller_than_acolumn_width_per_column = int(
        len_lines_bigger_than_x_width_smaller_than_acolumn_width / float(num_col_classifier))
        ...
        if len(all_args_uniq) > 0:
            if type(all_args_uniq[0]) is list:
                ...
                for dd in range(len(all_args_uniq)):
                    ...
                    if diff_max_min_uniques > sum_dis and ((sum_dis / float(diff_max_min_uniques)) > 0.85) and (
                            (diff_max_min_uniques / float(img_p_in_ver.shape[1])) > 0.85) and np.std(
                            dist_x_hor[some_args]) < (0.55 * np.mean(dist_x_hor[some_args])):
...
def find_number_of_columns_in_document(region_pre_p, num_col_classifier, tables, pixel_lines, contours_h=None):
    ...
    slope_linese, dist_xe, x_min_maine, x_max_maine, cy_maine, slope_lines_orge, y_min_maine, y_max_maine, cx_maine = find_features_of_lines(
        contours_line_e)
    ...
    slope_lines, dist_x, x_min_main, x_max_main, cy_main, slope_lines_org, y_min_main, y_max_main, cx_main = find_features_of_lines(
        contours_line_vers)
    ...
    if contours_h is not None:
        slope_lines_head, dist_x_head, x_min_main_head, x_max_main_head, cy_main_head, slope_lines_org_head, y_min_main_head, y_max_main_head, cx_main_head = find_features_of_lines(
            contours_h)
    ...
    cy_main_splitters = cy_main_hor[
        (x_min_main_hor <= .16 * region_pre_p.shape[1]) & (x_max_main_hor >= .84 * region_pre_p.shape[1])]

    cy_main_splitters = np.array(list(cy_main_splitters) + list(special_separators))

    if contours_h is not None:
        try:
            cy_main_splitters_head = cy_main_head[
                (x_min_main_head <= .16 * region_pre_p.shape[1]) & (x_max_main_head >= .84 * region_pre_p.shape[1])]
    ...
    for itiles in args_big_parts:
        ...
        regions_without_separators_tile = regions_without_separators[
                                          int(splitter_y_new[itiles]):int(splitter_y_new[itiles + 1]), :, 0]
...
def return_boxes_of_images_by_order_of_reading_new(splitter_y_new, regions_without_separators, matrix_of_lines_ch,
    ...
    for i in range(len(splitter_y_new) - 1):
        ...
        matrix_new = matrix_of_lines_ch[:, :][
            (matrix_of_lines_ch[:, 6] > splitter_y_new[i]) & (matrix_of_lines_ch[:, 7] < splitter_y_new[i + 1])]
        ...
        if 1 > 0:  # len( matrix_new[:,9][matrix_new[:,9]==1] )>0 and np.max(matrix_new[:,8][matrix_new[:,9]==1])>=0.1*(np.abs(splitter_y_new[i+1]-splitter_y_new[i] )):
                    ...
                    for i_n in range(len(peaks_neg_fin_early) - 1):
                        ...
                        try:
                            num_col, peaks_neg_fin1 = find_num_col(
                                regions_without_separators[int(splitter_y_new[i]):int(splitter_y_new[i + 1]),
                                peaks_neg_fin_early[i_n]:peaks_neg_fin_early[i_n + 1]], num_col_classifier, tables,
                                multiplier=7.)
                        except:
                            peaks_neg_fin1 = []
            ...
            if (reading_order_type == 1) or (
                ...
                try:
                    ...
                    y_type_2_up = np.array(y_type_2)[
                        (np.array(y_type_2) > int(splitter_y_new[i])) & (np.array(y_type_2) <= y_grenze)]
                    x_starting_up = np.array(x_starting)[
                        (np.array(y_type_2) > int(splitter_y_new[i])) & (np.array(y_type_2) <= y_grenze)]
                    x_ending_up = np.array(x_ending)[
                        (np.array(y_type_2) > int(splitter_y_new[i])) & (np.array(y_type_2) <= y_grenze)]
                    y_diff_type_2_up = np.array(y_diff_type_2)[
                        (np.array(y_type_2) > int(splitter_y_new[i])) & (np.array(y_type_2) <= y_grenze)]
                    args_up = args_early_ys[
                        (np.array(y_type_2) > int(splitter_y_new[i])) & (np.array(y_type_2) <= y_grenze)]

                    if len(y_type_2_up) > 0:
                        y_main_separator_up = y_type_2_up[
                            (x_starting_up == 0) & (x_ending_up == (len(peaks_neg_tot) - 1))]
                        y_diff_main_separator_up = y_diff_type_2_up[
                            (x_starting_up == 0) & (x_ending_up == (len(peaks_neg_tot) - 1))]
                        ...
                        if len(y_diff_main_separator_up) > 0:
                            ...
                            boxes.append([0, peaks_neg_tot[len(peaks_neg_tot) - 1], int(splitter_y_new[i]),
                                          int(np.max(y_diff_main_separator_up))])
                    ...
                    if (
                        ...
                        if reading_order_type == 1:
                            ...
                            for dj in range(len(x_start_without_mother)):
                                columns_covered_by_mothers = columns_covered_by_mothers + list(
                                    np.array(range(x_start_without_mother[dj], x_end_without_mother[dj])))
                        ...
                        for dj in range(len(x_end_with_child_without_mother)):
                            columns_covered_by_with_child_no_mothers = columns_covered_by_with_child_no_mothers + list(
                                np.array(
                                    range(x_start_with_child_without_mother[dj], x_end_with_child_without_mother[dj])))
                        ...
                        for i_s_nc in columns_not_covered_child_no_mother:
                            if i_s_nc in x_start_with_child_without_mother:
                                x_end_biggest_column = np.array(x_end_with_child_without_mother)[
                                    np.array(x_start_with_child_without_mother) == i_s_nc][0]
                                args_all_biggest_lines = ind_args[
                                    (x_starting == i_s_nc) & (x_ending == x_end_biggest_column)]
                                ...
                                for i_c in range(len(y_column_nc)):
                                    if i_c == (len(y_column_nc) - 1):
                                        ind_all_lines_betweeen_nm_wc = ind_args[
                                            (y_type_2 > y_column_nc[i_c]) & (y_type_2 < int(splitter_y_new[i + 1])) & (
                                                        x_starting >= i_s_nc) & (x_ending <= x_end_biggest_column)]
                                    else:
                                        ind_all_lines_betweeen_nm_wc = ind_args[
                                            (y_type_2 > y_column_nc[i_c]) & (y_type_2 < y_column_nc[i_c + 1]) & (
                                                        x_starting >= i_s_nc) & (x_ending <= x_end_biggest_column)]
                                    ...
                                    for dj in range(len(x_starting_all_between_nm_wc)):
                                        columns_covered_by_mothers = columns_covered_by_mothers + list(np.array(
                                            range(x_starting_all_between_nm_wc[dj], x_ending_all_between_nm_wc[dj])))
                                    columns_covered_by_mothers = list(set(columns_covered_by_mothers))
                                    ...
                                    if len(x_diff_all_between_nm_wc) > 0 and set(list(np.array(
                                            range(x_starting_all_between_nm_wc[biggest],
                                                  x_ending_all_between_nm_wc[biggest]))) + list(
                                            columns_not_covered)) != set(all_columns):
                                        should_longest_line_be_extended = 1

                                        index_lines_so_close_to_top_separator = np.array(range(len(y_all_between_nm_wc)))[
                                            (y_all_between_nm_wc > y_column_nc[i_c]) & (
                                                        y_all_between_nm_wc <= (y_column_nc[i_c] + 500))]

                                        if len(index_lines_so_close_to_top_separator) > 0:
                                            indexes_remained_after_deleting_closed_lines = np.array(list(
                                                set(list(np.array(range(len(y_all_between_nm_wc))))) - set(
                                                    list(index_lines_so_close_to_top_separator))))
                    ...
                    boxes.append(
                        [0, peaks_neg_tot[len(peaks_neg_tot) - 1], int(splitter_y_new[i]), int(splitter_y_new[i + 1])])
                ...
                if len(x_starting) > 0:
                    ...
                    for dj in range(len(x_starting)):
                        if set(list(np.array(range(x_starting[dj], x_ending[dj])))) == set(all_columns):
                            ...
                            columns_covered_by_lines_covered_more_than_2col = columns_covered_by_lines_covered_more_than_2col + list(
                                np.array(range(x_starting[dj], x_ending[dj])))
